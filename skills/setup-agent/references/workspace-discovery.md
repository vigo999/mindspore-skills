...
```bash
bash -lc 'source /usr/local/Ascend/ascend-toolkit/set_env.sh >/dev/null 2>&1 && HF_ENDPOINT=https://hf-mirror.com uv run --python <selected_python_path> python -c "from huggingface_hub import snapshot_download; snapshot_download(repo_id='\''org/model'\'', local_dir='\''./models/model'\'', local_dir_use_symlinks=False)"'
```
