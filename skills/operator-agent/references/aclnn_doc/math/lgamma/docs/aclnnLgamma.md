...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      <td>self和out的shape、数据格式或数据类型不一致。</td>
    </tr>
    <tr>
...
int main() {
  ...
  // 调用aclnnLgamma第一段接口
  ret = aclnnLgammaGetWorkspaceSize(self, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLgammaGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnLgamma第二段接口
  ret = aclnnLgamma(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLgamma failed. ERROR: %d\n", ret); return ret);
