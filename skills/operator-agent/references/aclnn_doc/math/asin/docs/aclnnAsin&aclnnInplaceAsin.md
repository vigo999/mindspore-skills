...
- aclnnAsin和aclnnInplaceAsin实现相同的功能，使用区别如下，请根据自身实际场景选择合适的算子。
  - aclnnAsin：需新建一个输出张量对象存储计算结果。
  - aclnnInplaceAsin：无需新建输出张量对象，直接在输入张量的内存中存储计算结果。

- 每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnAsinGetWorkspaceSize”或者“aclnnInplaceAsinGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAsin”或者“aclnnInplaceAsin”接口执行计算。
...
- **参数说明**：
  ...
  - self(aclTensor*,计算输入)：Device侧的aclTensor，当类型为INT8、INT16、INT32、 INT64、 UINT8、BOOL时，转化为FLOAT32进行运算，输出FLOAT32类型。维度不大于8。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，支持空Tensor，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持INT8、INT16、INT32、INT64、UINT8、BOOL、FLOAT、FLOAT16、DOUBLE、BFLOAT16，当类型为BFLOAT16时，转化为FLOAT16进行计算，再转化为BFLOAT16输出。
  - out(aclTensor\*，计算输出)：Device侧的aclTensor，维度不大于8，且shape需要与self一致。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，支持空Tensor，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnAsinGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- **参数说明：**
  ...
  * selfRef(aclTensor *，计算输入|计算输出)：公式中的输入`input`与输出`out`，维度不大于8。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，支持空Tensor，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT、FLOAT16、DOUBLE、BFLOAT16。 
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnInplaceAsinGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnAsin&aclnnInplaceAsin默认确定性实现。
