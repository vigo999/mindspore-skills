...
[📄 查看源码](https://gitcode.com/cann/ops-math/tree/master/conversion/reflection_pad3d_grad)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnReflectionPad3dBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnReflectionPad3dBackward”接口执行计算。
...
- **参数说明**
  ...
  <tbody>
    <tr>
      ...
      <td>输入</td>
      <td>反向传播的输入。</td>
      <td>shape需要与reflection_pad3d正向传播的output一致。</td>
      <td>与self保持一致</td>
      <td>ND</td>
      <td>4/5</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>-</td>
      <td><ul><li>长度为6，数值依次代表左右上下前后需要填充的值。</li><li>padding前两个数值需小于self最后一维度的数值，中间两个数值需小于self倒数第二维度的数值，后两个数值需小于self倒数第三维度的数值。</li></ul></td>
      <td>INT64</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
...
- **返回值**  
  ...
  <tbody>
    ...
    <tr>
      <td>gradOutput shape需要与reflection_pad3d正向传播的output一致。</td>
    </tr>
  </tbody>
  </table>
