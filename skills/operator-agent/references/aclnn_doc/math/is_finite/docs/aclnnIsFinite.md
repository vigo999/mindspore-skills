...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnIsFiniteGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  ret = aclnnIsFiniteGetWorkspaceSize(self, out, &workspaceSize, &executor);
  ...
  ret = aclnnIsFinite(workspaceAddr, workspaceSize, executor, stream);
