...
每个算子分为[两段式接口](../../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnLogicalAndGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnLogicalAnd”接口执行计算。
...
int main()
{
    ...
    // 创建other aclTensor
    ret = CreateAclTensor(otherHostData, otherShape, &otherDeviceAddr, aclDataType::ACL_BOOL, &other);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ret = CreateAclTensor(outHostData, outShape, &outDeviceAddr, aclDataType::ACL_BOOL, &out);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 3. 调用CANN算子库API，需要修改为具体的API名称
    // aclnnLogicalAnd接口调用示例
    ...
    std::vector<bool> resultData(size, 0);
    ret = aclrtMemcpy(
    ...
    for (int64_t i = 0; i < size; i++) {
        if (resultData[i]) {
            LOG_PRINT("result result[%ld] is: true\n", i);
        } else {
            LOG_PRINT("result result[%ld] is: false\n", i);
        }
    }
