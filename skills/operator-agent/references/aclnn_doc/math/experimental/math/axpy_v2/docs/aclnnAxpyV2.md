...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>x1的数据类型和数据格式不在支持的范围之内。</td>
    </tr>
    <tr>
      <td>x1的数据维度超过了8维。</td>
    </tr>
    <tr>
      <td>x1和out的数据形状不一致。</td>
    </tr>
  </tbody></table>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnAxpyV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  std::vector<float> otherHostData = {0, 1, 2, 3};
  ...
  alpha = aclCreateScalar(&scalarValue, ACL_FLOAT);
  // 创建out aclTensor
  ...
  aclOpExecutor* executor;
  // 调用aclnnAxpyV2第一段接口
  ret = aclnnAxpyV2GetWorkspaceSize(self, other, alpha, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAxpyV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnAxpyV2第二段接口
  ret = aclnnAxpyV2(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAxpyV2 failed. ERROR: %d\n", ret); return ret);
  // 4. （固定写法）同步等待任务执行结束
  ...
  for (int64_t i = 0; i < size; i++) {
    LOG_PRINT("aclnnAxpyV2 result[%ld] is: %f\n", i, resultData[i]);
  }
