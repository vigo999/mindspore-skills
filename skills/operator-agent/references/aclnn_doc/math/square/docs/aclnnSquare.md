...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      <td>输入self和输出out的数据类型、shape不一致。</td>
    </tr>
  </tbody>
  </table>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnSquareGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_square.h"
...
int main() {
  ...
  std::vector<float> outHostData = {0, 1, 4, 9, 16, 25, 36, 49};
  // 创建self aclTensor
  ...
  aclOpExecutor* executor;
  // 调用aclnnSquare第一段接口
  ret = aclnnSquareGetWorkspaceSize(self, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSquareGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnSquare第二段接口
  ret = aclnnSquare(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSquare failed. ERROR: %d\n", ret); return ret);
