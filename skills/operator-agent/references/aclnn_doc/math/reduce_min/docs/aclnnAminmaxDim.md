...
返回输入张量在指定维度上的最小值和最大值。

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnAminmaxDimGetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnAminmaxDim”接口执行计算。
...
- **返回值：**
  ...
  <tbody>
    <tr>
      ...
      <td>161001</td>
      <td>传入的self、minOut或maxOut是空指针。</td>
    </tr>
    <tr>
...
- **参数说明**
    ...
    <tbody>
      ...
      <tr>
        <td>workspaceSize</td>
        <td>输入</td>
        <td>在Device侧申请的workspace大小，由第一段接口aclnnAminmaxDimGetWorkspaceSize获取。</td>
      </tr>
      <tr>
        <td>executor</td>
        <td>输入</td>
