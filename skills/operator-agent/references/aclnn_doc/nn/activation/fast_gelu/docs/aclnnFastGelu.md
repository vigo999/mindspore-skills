...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_fast_gelu.h"
...
int main()
{
    ...
    aclOpExecutor* executor;
    // 调用aclnnFastGelu第一段接口
    ret = aclnnFastGeluGetWorkspaceSize(self, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnFastGeluGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用aclnnFastGelu第二段接口
    ret = aclnnFastGelu(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnFastGelu failed. ERROR: %d\n", ret); return ret);
    // 4. （固定写法）同步等待任务执行结束
