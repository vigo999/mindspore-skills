...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/activation/gelu)
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnGeluGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGeluGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGelu failed. ERROR: %d\n", ret); return ret);
  // 4. （固定写法）同步等待任务执行结束
