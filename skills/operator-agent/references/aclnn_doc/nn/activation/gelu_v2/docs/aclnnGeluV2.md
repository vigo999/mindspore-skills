...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/activation/gelu_v2)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnGeluV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnGeluV2”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
      ...
      <td><ul><li>数据类型必须和y一致。</li><li>shape必须和y一致。</li><li>支持空Tensor。</li></ul></td>
      ...
      <td>输出</td>
      <td>表示GELU激活函数的输出。</td>
      <td><ul><li>数据类型必须和x一致。</li><li>shape必须和x一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnGeluV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
