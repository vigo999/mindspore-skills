...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/activation/hard_shrink)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnHardshrinkGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnHardshrink”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>out（aclTensor*）</td>
      <td>输出</td>
      <td>公式中的Hardshrink(x)。</td>
      <td><ul><li>当输入x > λ或者x < -λ时，输出为x，其他情况输出0。</li><li>shape需要与self一致。</li><li>支持空Tensor。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnHardshrinkGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnHardshrink第一段接口
  ret = aclnnHardshrinkGetWorkspaceSize(self, lambda, out, &workspaceSize, &executor);
