...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_copy)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachCopyGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachCopy”接口执行计算。
...
- **参数说明**：
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行内容复制的输入张量列表，对应公式中的`x`。</td>
      <td><ul><li>支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16、INT8、UINT8、INT16、UINT16、INT32、UINT32、INT64、DOUBLE、BOOL</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
    </tr>
    <tr>
      ...
      <td>输出</td>
      <td>表示进行内容复制的输出张量列表，对应公式中的`y`。</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachCopyGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main()
{
    ...
    aclOpExecutor *executor;
    // 调用aclnnForeachCopy第一段接口
    ret = aclnnForeachCopyGetWorkspaceSize(tensorListInput, tensorListOutput, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachCopyGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用aclnnForeachCopy第二段接口
    ret = aclnnForeachCopy(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachCopy failed. ERROR: %d\n", ret); return ret);
    // 4. （固定写法）同步等待任务执行结束
