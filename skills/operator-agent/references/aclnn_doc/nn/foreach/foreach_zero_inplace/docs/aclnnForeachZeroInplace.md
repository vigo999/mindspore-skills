...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_zero_inplace)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachZeroInplaceGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachZeroInplace”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachZeroInplaceGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachZeroInplace默认确定性实现。
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachZeroInplace第一段接口
  ...
  // 调用aclnnForeachZeroInplace第二段接口
