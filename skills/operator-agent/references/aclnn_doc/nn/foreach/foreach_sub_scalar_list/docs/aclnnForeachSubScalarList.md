...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_sub_scalar_list)
...
- 接口功能：对输入张量列表的每个张量与标量列表scalars的每个标量逐元素执行相减运算。
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachSubScalarListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachSubScalarList”接口执行计算。
...
- **参数说明**：
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示减法运算的第一个输入张量列表，对应公式中的`x`。</td>
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachSubScalarListGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachSubScalarList第一段接口
  ret = aclnnForeachSubScalarListGetWorkspaceSize(tensorListInput, scalarlist, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachSubScalarListGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachSubScalarList第二段接口
  ret = aclnnForeachSubScalarList(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachSubScalarList failed. ERROR: %d\n", ret); return ret);
