...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_lerp_list)
...
- 计算公式：
  ...
  weight = [{weight_0}, {weight_1}, ... {weight_{n-1}}]\\
  y = [{y_0}, {y_1}, ... {y_{n-1}}]\\
  $$

  $$
  {\rm y}_i = x1_i + {\rm weight}_i × (x2_i - x1_i) (i=0,1,...n-1)
  $$

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachLerpListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachLerpList”接口执行计算。
...
- **参数说明**：
  ...
  <tbody>
    <tr>
      ...
      <td>输入</td>
      <td>表示进行线性插值计算的第一个输入张量列表。对应公式中的`x1`。</td>
      <td><ul><li>不支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>shape与入参`x2`、`weight`的shape一致。</li></ul></td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行线性插值计算的第二个输入张量列表。对应公式中的`x2`。</td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行线性插值计算的插值系数张量列表。对应公式中的`weight`。</td>
      <td><ul><li>不支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>数据类型、数据格式和shape与入参`x1`的数据类型、数据格式和shape一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>表示进行线性插值计算的输出张量列表。对应公式中的`y`。</td>
...
- **返回值**：
  ...
  <tbody>
    <tr>
      ...
      <td>161001</td>
      <td>传入的x1、x2、weight、out是空指针。</td>
    </tr>
    <tr>
      <td rowspan="2">ACLNN_ERR_PARAM_INVALID</td>
      <td rowspan="2">161002</td>
      <td>x1、x2、weight、out的数据类型不在支持的范围之内。</td>
    </tr>
    <tr>
      <td>x1、x2、weight、out的数据类型不一致。</td>
    </tr>
    <tr>
      <td rowspan="3">ACLNN_ERR_INNER_TILING_ERROR</td>
      <td rowspan="3">561002</td>
      <td>x1、x2、weight、out的shape不满足约束。</td>
    </tr>
    <tr>
      <td>x1、x2、weight或out中的Tensor的数据类型不一致。</td>
    </tr>
    <tr>
      <td>x1、x2、weight或out中的Tensor维度超过8维。</td>
    </tr>
  </tbody></table>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachLerpListGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  std::vector<int64_t> weightShape1 = {2, 3};
  std::vector<int64_t> weightShape2 = {1, 3};
  ...
  std::vector<float> weight1HostData = {1, 2, 3, 4, 5, 6};
  std::vector<float> weight2HostData = {7, 8, 9};
  ...
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  // 创建weight1 aclTensor
  ret = CreateAclTensor(weight1HostData, weightShape1, &weight1DeviceAddr, aclDataType::ACL_FLOAT, &weight1);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  // 创建weight2 aclTensor
  ...
  std::vector<aclTensor*> tempWeight{weight1, weight2};
  aclTensorList* tensorListWeight = aclCreateTensorList(tempWeight.data(), tempWeight.size());
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachLerpList第一段接口
  ret = aclnnForeachLerpListGetWorkspaceSize(tensorListInput1, tensorListInput2, tensorListWeight, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachLerpListGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachLerpList第二段接口
  ret = aclnnForeachLerpList(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachLerpList failed. ERROR: %d\n", ret); return ret);
  ...
  aclDestroyTensorList(tensorListWeight);
