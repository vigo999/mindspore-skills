...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_tanh)
...
- 计算公式：
  ...
  $$
  y_i=\frac{e^{x_i}-e^{-x_i}}{e^{x_i}+e^{-x_i}} (i=0,1,...n-1)
  $$

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachTanhGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachTanh”接口执行计算。
...
## aclnnForeachTanhGetWorkspaceSize
...
<table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行双曲正切函数运算的输入张量列表，对应公式中的`x`。</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>表示进行双曲正切函数运算的输出张量列表，对应公式中的`y`。</td>
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachTanhGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachTanh第一段接口
  ret = aclnnForeachTanhGetWorkspaceSize(tensorListInput, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachTanhGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachTanh第二段接口
  ret = aclnnForeachTanh(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachTanh failed. ERROR: %d\n", ret); return ret);
