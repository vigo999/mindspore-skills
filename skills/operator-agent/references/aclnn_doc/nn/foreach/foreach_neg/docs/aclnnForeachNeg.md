...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_neg)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachNegGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachNeg”接口执行计算。
...
## aclnnForeachNegGetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachNegGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachNeg第一段接口
  ret = aclnnForeachNegGetWorkspaceSize(tensorListInput, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachNegGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachNeg第二段接口
  ret = aclnnForeachNeg(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachNeg failed. ERROR: %d\n", ret); return ret);
