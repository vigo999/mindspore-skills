...
- 接口功能：对张量列表x和标量列表scalars执行逐元素除法。
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachDivScalarListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachDivScalarList”接口执行计算。
...
- **参数说明**：
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行除法运算的第二个输入标量列表，对应公式中的`scalars`。</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>表示进行除法运算的输出张量列表，对应公式中的`y`。</td>
      <td><ul><li>支持空Tensor。</li><li>`out`的TensorList长度需要与`x`的TensorList长度相等。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>数据类型和数据格式与入参`x`的数据类型和数据格式一致，shape size大于等于入参`x`的shape size。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
...
- **返回值**：
  ...
  <tbody>
    ...
    <tr>
      <td>x的TensorList长度不在支持范围。</td>
    </tr>
    <tr>
      <td>x与scalars或out的TensorList长度不相等。</td>
    </tr>
  </tbody></table>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachDivScalarListGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachDivScalarList默认确定性实现。
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachDivScalarList第一段接口
  ret = aclnnForeachDivScalarListGetWorkspaceSize(tensorListInput, scalarlist, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachDivScalarListGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachDivScalarList第二段接口
  ret = aclnnForeachDivScalarList(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachDivScalarList failed. ERROR: %d\n", ret); return ret);
