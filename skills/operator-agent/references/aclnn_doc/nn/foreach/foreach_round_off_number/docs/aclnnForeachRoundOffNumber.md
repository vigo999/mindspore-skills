...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachRoundOffNumberGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachRoundOffNumber”接口执行计算。
...
- **返回值**：
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>x、roundMode、out的数据类型不在支持的范围之内。</td>
    </tr>
    <tr>
      <td>x和out的数据类型不一致。</td>
    </tr>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachRoundOffNumberGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  std::vector<float> alphaHostData = {1};
  // 创建input1 aclTensor
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachRoundOffNumber第一段接口
  ...
  // 调用aclnnForeachRoundOffNumber第二段接口
