...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_round_off_number)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachRoundOffNumberV2GetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachRoundOffNumberV2”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachRoundOffNumberV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachRoundOffNumberV2默认确定性实现。
...
int main() {
  ...
  std::vector<float> input1HostData = {1.1, 2.2, 3.3, 4.5, 5.3, 6.12};
  std::vector<float> input2HostData = {7.45, 8.87, 9.54};
  ...
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  // 创建alpha aclScalar
  alpha = aclCreateScalar(&alphaValue, aclDataType::ACL_INT8);
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachRoundOffNumberV2第一段接口
  ret = aclnnForeachRoundOffNumberV2GetWorkspaceSize(tensorListInput, alpha, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachRoundOffNumberV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachRoundOffNumberV2第二段接口
  ret = aclnnForeachRoundOffNumberV2(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachRoundOffNumberV2 failed. ERROR: %d\n", ret); return ret);
