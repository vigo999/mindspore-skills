...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_addcmul_list)
...
- 接口功能：先对张量列表x2和张量列表x3执行逐元素乘法，并将结果乘以张量scalars，最后将之前计算的结果与张量列表x1执行逐元素相加。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachAddcmulListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachAddcmulList”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachAddcmulListGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachAddcmulList默认确定性实现。
...
int main() {
  ...
  std::vector<float> scalarHostData = {1, 2};
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachAddcmulList第一段接口
  ret = aclnnForeachAddcmulListGetWorkspaceSize(tensorListInput1, tensorListInput2, tensorListAnother, scalar, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachAddcmulListGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachAddcmulList第二段接口
  ret = aclnnForeachAddcmulList(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachAddcmulList failed. ERROR: %d\n", ret); return ret);
