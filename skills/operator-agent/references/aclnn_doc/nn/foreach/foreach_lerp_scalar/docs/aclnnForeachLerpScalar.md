...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_lerp_scalar)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachLerpScalarGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachLerpScalar”接口执行计算。
...
- **参数说明**：
  ...
  <tbody>
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>数据类型和数据格式与入参`x2`的数据类型和数据格式保持一致。</li></ul></td>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>数据类型和数据格式与入参`x1`的数据类型和数据格式保持一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachLerpScalarGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  std::vector<float> other1HostData = {4, 6, 7, 8, 4, 3};
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachLerpScalar第一段接口
  ret = aclnnForeachLerpScalarGetWorkspaceSize(tensorListInput1, tensorListInput2, weight, tensorListOutput, &workspaceSize, &executor);
