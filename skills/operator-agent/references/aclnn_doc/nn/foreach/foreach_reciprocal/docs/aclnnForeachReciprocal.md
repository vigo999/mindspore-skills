...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_reciprocal)
...
- **参数说明**：
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示进行倒数运算的输入张量列表，对应公式中的`x`。</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>表示进行倒数运算的输出张量列表，对应公式中的`y`。</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachReciprocalGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachReciprocal第一段接口
  ret = aclnnForeachReciprocalGetWorkspaceSize(tensorListInput, tensorListOutput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachReciprocalGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachReciprocal第二段接口
  ret = aclnnForeachReciprocal(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachReciprocal failed. ERROR: %d\n", ret); return ret);
