...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_add_scalar_list)
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnForeachAddScalarListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachAddScalarList”接口执行计算。
...
- **参数说明**：
  ...
  <tbody>
    <tr>
      ...
      <td>输入</td>
      <td>表示进行加法运算的输入张量列表。对应公式中的`x`。</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>表示进行加法运算的输出张量列表。对应公式中的`y`。</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachAddScalarListGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
