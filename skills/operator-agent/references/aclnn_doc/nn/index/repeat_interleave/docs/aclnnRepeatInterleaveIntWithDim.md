...
## 功能说明
  - 算子功能：将tensor中的每个元素根据dim这个维度，每个元素重复repeats的相应次数。

  - 示例：
    假设input tensor是 ([[a, b], [c, d], [e, f]])。repeats为2, dim为0。
    那么最后生成的tensor为 [ [a, b], [a, b], [c, d], [c, d], [e, f], [e, f]]。
    在dim 0维度，对应的a、b会重复2次，对应的c、d会重复2次，对应的e、f会重复2次。

  - 在计算时需要满足要求：
    outputSize = self的dim维度的size * repeats的值。

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnRepeatInterleaveIntWithDimGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnRepeatInterleaveIntWithDim”接口执行计算。
...
- **参数说明：**
  ...
  - self(aclTensor*, 计算输入)：Device侧的aclTensor，功能说明中待被数据复制的输入tensor。支持空tensor, 支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)。[数据格式](../../../docs/zh/context/数据格式.md)支持ND，shape支持1-8维。
  ...
  - outputSize(int64_t, 计算输入)：dim维度再进行重复后的最终大小。Host侧的int64_t类型。outputSize值必须为repeats * self的dim维度size。

  - out(aclTensor*, 计算输出)：Device侧的aclTensor，功能说明中数据复制完成的输出tensor。数据类型需要与self一致，[数据格式](../../../docs/zh/context/数据格式.md)支持ND，shape维度数量与self一致，out的shape在dim维度下的size等于repeats * self在dim维度下的size。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持UINT8、INT8、INT16、INT32、INT64、BOOL、FLOAT16、BFLOAT16、FLOAT类型。
...
- **返回值：**
  ...
  返回161002 (ACLNN_ERR_PARAM_INVALID): 1. self的数据类型不在支持范围内。
                                        2. self、out的数据类型不一样。
                                        3. repeats不为自然数。
                                        4. dim的值不在[-self的维度数量， self的维度数量-1]之间。
                                        5. self的维度数超过8。
                                        6. 当self为0维时，不支持传入dim。
  ```
...
- **参数说明：**
  ...
  - workspaceSize(uint64_t, 入参)：在Device侧申请的workspace大小，由第一段接口aclnnRepeatInterleaveIntWithDimGetWorkspaceSize获取。
...
- 确定性计算：
  - aclnnRepeatInterleaveIntWithDim默认确定性实现。

在计算时需要满足要求：outputSize = self的dim维度的size * repeats的值。
...
int main() {
  ...
  // 调用aclnnRepeatInterleaveIntWithDim第一段接口
  ...
  // 调用aclnnRepeatInterleaveIntWithDim第二段接口
