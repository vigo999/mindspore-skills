...
- 接口功能：沿输入self的给定轴dim，将index指定位置的值使用value进行替换，计算结果是确定的。
- 示例：输入self为：
...
- aclnnIndexFill和aclnnInplaceIndexFill实现相同的功能，使用区别如下，请根据自身实际场景选择合适的算子。
    - aclnnIndexFill：需新建一个输出张量对象存储计算结果。
    - aclnnInplaceIndexFill：无需新建输出张量对象，直接在输入张量的内存中存储计算结果。
- 每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnIndexFillGetWorkspaceSize”或者“aclnnInplaceIndexFillGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnIndexFill”或者“aclnnInplaceIndexFill”接口执行计算。
...
- **参数说明**
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>其中的元素值小于self对应dim的维度大小，支持负数，支持越界，对于越界值会直接跳过。</td>
        <td>INT32、INT64</td>
        <td>-</td>
        <td>-</td>
        <td>-</td>
...
- **参数说明**
  ...
  <table style="undefined;table-layout: fixed; width: 1244px"><colgroup>
      ...
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnIndexFillGetWorkspaceSize获取。</td>
      </tr>
      <tr>
      <td>executor</td>
      <td>输入</td>
...
- **参数说明**
  ...
  <table style="undefined;table-layout: fixed; width: 1244px"><colgroup>
      ...
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnInplaceIndexFillGetWorkspaceSize获取。</td>
      </tr>
      <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnIndexFill&aclnnInplaceIndexFill默认确定性实现。
