#!/usr/bin/env python3
"""Offline parser for Ascend PyTorch Profiler data (*_ascend_pt).

Calls torch_npu.profiler.profiler.analyse() to generate ASCEND_PROFILER_OUTPUT
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Offline parser for Ascend PyTorch Profiler data (*_ascend_pt)."
    )
    parser.add_argument(
        "profiler_path",
        type=str,
        help="Path to the profiler result directory (e.g., ./result_data_ascend_pt)",
    )
    ...
    try:
        from torch_npu.profiler.profiler import analyse  # type: ignore[import-untyped]
    except ImportError as e:
        print(f"Error: torch_npu not installed or analyse unavailable: {e}", file=sys.stderr)
        return 1
